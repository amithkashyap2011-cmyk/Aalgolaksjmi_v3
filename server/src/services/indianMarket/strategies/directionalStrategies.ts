...
export class LongCallStrategy extends BaseStrategy {
  ...
  public constructTrade(
    ...
    const optionPremium = OptionChainService.markPrice(context.underlying, context.spotPrice, strikeInfo.strike, true, expiryInfo.expiry);
...
export class LongPutStrategy extends BaseStrategy {
  ...
  public constructTrade(
    ...
    const dteYears = Math.max(0.5, (expiryInfo.date.getTime() - Date.now()) / (1000 * 60 * 60 * 24)) / 365;
    const optionPremium = OptionChainService.markPrice(context.underlying, context.spotPrice, strikeInfo.strike, false, expiryInfo.expiry);
