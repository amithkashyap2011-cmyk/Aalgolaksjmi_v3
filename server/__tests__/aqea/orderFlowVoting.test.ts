...
let AQEAEngine: any, AQEA_CONFIG: any, Settings: any, RegimeEngine: any;
beforeAll(async () => {
...
describe("AQEA Phase 2B: Order Flow Voting Integration", () => {
  ...
  beforeEach(() => {
    ...
    (AQEA_CONFIG as any).SMART_MONEY_VOTING_ENABLED = false;
  ...
  test("Order Flow receives ~11% normalized weight in TRENDING_BULL", async () => {
    const res = await AQEAEngine.decide(symbol, userId, baseContext);
    expect(res.meta.weightsApplied.orderFlow).toBeCloseTo(0.105, 2);
    expect(res.reasons.some((r: string) => r.includes("OF_WEIGHT"))).toBe(true);
  });

  test("Order Flow scales to ~21% normalized weight in HIGH_VOLATILITY", async () => {
    if (RegimeEngine?.analyze) {
      jest.spyOn(RegimeEngine, "analyze").mockReturnValue({ state: "HIGH_VOLATILITY", score: 50, confidence: 50 } as any);
    }
    ...
    mockOrderFlowAnalyze.mockResolvedValue({ votingScore: 70, pressure: "BUY", diagnostics: {} });

    const res = await AQEAEngine.decide(symbol, userId, baseContext);
    expect(res.meta.weightsApplied.orderFlow).toBeCloseTo(0.21, 2);
    expect(res.reasons.some((r: string) => r.includes("OF_WEIGHT"))).toBe(true);
  });

  test("Rollback: ORDERFLOW_VOTING_ENABLED=false assigns 0% weight", async () => {
    (AQEA_CONFIG as any).ORDERFLOW_VOTING_ENABLED = false;

    const res = await AQEAEngine.decide(symbol, userId, baseContext);
    expect(res.meta.weightsApplied.orderFlow).toBe(0);
    expect(res.meta.weightsApplied.core).toBe(1.0);
  });
});
